...
class ACFQLAgent(flax.struct.PyTreeNode):
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        u ,dudt = jax.jvp(cond_mean_flow,(x_t,t_begin,t_end),(x_1-x_0,jnp.zeros_like(t_begin),jnp.ones_like(t_end)))
        if self.config["action_chunking"]:
            # === 【JVP Mask 修复 V2】 ===
            # 1. 获取相关维度信息
        ...
        pred = self.network.select('actor_bc_flow')(batch['observations'], x_t, t_begin,t_end, params=grad_params)
        ...
        if self.config["action_chunking"]:
            ...
            bc_flow_loss = jnp.mean(jnp.square(pred - u_tgt))
...
def get_config():
    ...
    config = ml_collections.ConfigDict(
        dict(
            agent_name='acmfql_bc',  # Agent name.
